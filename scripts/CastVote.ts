...
async function main() {
  ...
  console.log("ballotContractAddress", ballotContractAddress, proposalNumber);
  ...
  if (!proposalObj) throw new Error("Proposal not found");
  const proposal = hexToString(proposalObj[0] as `0x${string}`, { size: 32 });
  console.log(
    `Voting on proposal ${proposalNumber} ${proposal} with ${amount} tokens`
  );
